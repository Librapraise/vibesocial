...
export default function Layout({ children, currentPageName }: { children: React.ReactNode; currentPageName?: string }) {
  ...
  const adminLinks = [
    { name: "AdminDashboard", label: "Admin Panel", icon: ShieldCheck },
  ];
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex text-zinc-100">
      ...
      <div className="flex flex-col flex-1 min-w-0 pb-16 md:pb-0">
        {isDemoMode && (
          <div className="bg-gradient-to-r from-orange-500 via-pink-500 to-purple-600 text-white py-2.5 px-4 text-xs font-bold text-center flex items-center justify-center gap-2 relative z-50 shadow-md">
            <span>✨ You are exploring VibeSocial in Guest Demo Mode (purely local & sandboxed).</span>
            <button onClick={() => logout()} className="underline hover:text-zinc-200 ml-2 font-black transition-colors">
              Sign Out & Sign Up
            </button>
          </div>
        )}
        <header className="h-16 border-b border-zinc-900 bg-zinc-950/50 backdrop-blur-md sticky top-0 z-20 px-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              ...
              className="md:hidden p-2 -ml-2 text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-900 transition-colors mr-1"
      ...
      {/* Mobile Drawer Menu */}
      <div
        className={`fixed inset-y-0 left-0 w-72 bg-zinc-950 border-r border-zinc-900 z-50 md:hidden transition-transform duration-300 ease-in-out flex flex-col justify-between ${
      ...
      >
        <div className="h-16 flex items-center justify-between px-6 border-b border-zinc-900/50">
          ...
          <button
            onClick={() => setIsDrawerOpen(false)}
            className="text-zinc-400 hover:text-white p-1.5 hover:bg-zinc-900 rounded-lg transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
